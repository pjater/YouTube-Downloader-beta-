...
function openCookieWizard() {
    wizardCurrentStep = 1;
    updateWizardUI();
    const overlay = document.getElementById('cookie-wizard');
    overlay.style.display = 'flex';
    setTimeout(() => {
        overlay.classList.add('active');
    }, 10);
...
function updateWizardUI() {
    ...
    // Update step text
    document.getElementById('wizard-current-step').textContent = wizardCurrentStep;
    ...
    if (wizardCurrentStep === wizardTotalSteps) {
        ...
        `;
        nextBtn.onclick = closeCookieWizard;
    } else {
        nextBtn.innerHTML = `
            <span>Next</span>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
